...
const checkSignupValidations = async (req, res, next) => {
  try {
    ...
    if (!firstName || !emailId || !password) {
      return res
        .status(400)
        .json({ error: "First name, email, password are required" });
    } else if (firstName.length < 3 || firstName.length > 20) {
      return res
        .status(400)
        .json({ error: "First name must be between 3 and 20 characters" });
    }
    if ((lastName && lastName.length < 3) || lastName.length > 20) {
      return res
        .status(400)
        .json({ error: "Last name must be between 3 and 20 characters" });
    }
...
const checkUpdateValidations = (req, res, next) => {
  try {
    ...
    if (
      gender !== undefined &&
      !GENDER_OPTIONS.includes(String(gender).toLowerCase())
    ) {
      return res.status(400).json({ error: "Invalid gender" });
    }
